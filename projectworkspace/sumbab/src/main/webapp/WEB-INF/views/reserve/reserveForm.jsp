...
<!-- 가게 상세보기 view에서 storeVO.reserving==false 인 곳은 예약하기 버튼 비활성화 -->
	...
	<h1>${storelist.name} 예약하기</h1><br><br>
		...
		<table class="table table-dark">
			<tr>
				...
				<td><input type="text" name="reserveName" id="reserveName"
					placeholder="예약하시는 분의 성함을 입력해 주세요. " required><br>
	...
	<script>
		function f_check() {
			if (document.reserveDTO.usingID.checked == true) {
				document.reserveDTO.reserveName.value = "<%=(String)session.getAttribute("id")%>";
			} else if (document.reserveDTO.usingID.checked == false) {
				document.reserveDTO.reserveName.value ="";
			}
		}
	</script>
