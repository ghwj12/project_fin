...
function CheckLoginStatus(){
	var storeNum = ${storelist.storeNum};
	var id ='<%=session.getAttribute("id")%>';
	if(id =="null"){
		alert("로그인이 필요한 서비스 입니다. 회원가입 후 이용 가능합니다. ");
	}else{
		location.replace("/sumbab/reserve/reserveForm/"+storeNum);
	}
}
</script>
...
<jsp:include page="/WEB-INF/views/header.jsp" flush="true"/>
	...
	<c:choose>
		<c:when test="${storelist.picture != null}">
			<img src="<c:url value="/upload_01store/${storelist.picture}"/>"
				style="height: 250px; float: center;" />
			<%-- </c:when>
			<c:when test="${fn:length(Reviewlist) > 0}"> --%>
				<c:forEach var="reviewImgs" items="${Reviewlist}" varStatus="status" end="2">
					<c:if test="${reviewImgs.picture != null}">
						<img src="<c:url value="/upload_02review/${reviewImgs.picture}"/>" style="height:250px;"/>
					</c:if>
				</c:forEach>
			</c:when>
		<c:otherwise>
	...
	function openPick(storeNum){
		...
		url="pick/pickProcess/" + storeNum;						
		var openWin = window.open(url, "openPick", "left="+popX+",top="+popY+",width="+popWidth+",height="+popHeight);
	}
	</script>


	<input type="button" class="button" onclick="openPick(${storelist.storeNum})"
		value="보관함에담기">
	<c:set var="reserving" value="${storelist.reserving}" />
	<c:choose>
		<%--reserving 값에 따라 예약하기/예약불가 버튼 출력--%>
		<c:when test="${reserving == '1'}">
			<input type="button" class="button" value="예약하기"
				onclick="CheckLoginStatus();">
		</c:when>
		<c:otherwise>
			<input type="button" class="button" value="예약불가">
		</c:otherwise>
	</c:choose>
	<br><br>
	<!-- ================================================================================================================================================== -->
	<div id="reviewTitle">
		...
		<input type="button" value="리뷰 작성" class="button"
	...
	<div id="reviewList">
		<c:forEach items="${Reviewlist}" var="Reviewlist">
			<table class="reviewInfo">
				<tr>
					<th>작성자</th>
					<td width="500" style="word-break: break-all">&nbsp;${Reviewlist.id}
				...
				<tr>
					...
					<c:choose>
						<c:when test="${Reviewlist.picture eq null}">
						${Reviewlist.content}
						</c:when>
						<c:otherwise>
						<img src="/sumbab/upload_02review/${Reviewlist.picture}" width="220px" height="220px"><br>
							${Reviewlist.content}
						</c:otherwise>
					</c:choose>
					</td>
	...
	$(document).on("click", ".warning", function(e){
		...
		if(id =="null"){
			...
			var popX = winX + (winWidth - popWidth) / 2;
			var popY = winY + (winHeight - popHeight) / 2;
			url = "/sumbab/storeMerge/warningProcess/" + reviewNum
			var openWin = window.open(url, "openWarning", "left=" + popX
					+ ",top=" + popY + ",width=" + popWidth + ",height=" + popHeight);
		}
	});
		...
		$.ajax({
			...
			success:function(data){
				...
				$(data).each(function(i, elem){
					var img = $("<img>").attr({src: "/sumbab/upload_02review/" + elem.picture, width:"220px", height:"220px"});
					var div;
					if(elem.picture != null){
						div = $("<td>", {html:"<br />" + elem.content, css:{width:"500px", "word-break":"break-all"}}).prepend(img);
					}else{
						div = $("<td>", {html: elem.content, css:{width:"500px", "word-break":"break-all"}});
					}
					table
						.append($("<tr>").append($("<th>").text("작성자")).append($("<td>").html(elem.id+"&nbsp;")
								.append($("<input>", {type:"button", value:"신고하기", "class":"button"}).addClass("warning"))
						...
						.append($("<tr>")
							.append($("<th>").text("리뷰내용")).append(div));
	...
	<div id="nbsCafeList" class="bottomright"><br>
	...
	<div id="map" class="topcorner"
		style="width: 30%; height: 250px; float: left; margin-top:56px;"></div>
	<script type="text/javascript"
